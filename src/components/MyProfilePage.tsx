    ...
    try {
      ...
      if (saved !== null) {
        ...
        if (Array.isArray(parsed)) {
          return parsed.filter((p: CheckInPost) => !p.id.startsWith('post-'));
        }
      }
    } catch (e) {
      ...
      <div className="bg-slate-900 border border-slate-800 text-white p-5 rounded-2xl shadow-xl space-y-3.5">
        ...
        <div className="space-y-2.5">
          {displayedPosts.length === 0 ? (
            <div className="p-8 text-center bg-slate-800/50 rounded-xl border border-slate-700/50 text-slate-400 space-y-1.5">
              <MessageSquare className="w-8 h-8 mx-auto text-slate-600" />
              <p className="text-xs font-bold text-slate-300">目前尚無留言紀錄</p>
              <p className="text-[10px] text-slate-400">在上方輸入您的運動心得，發布第一則社群分享吧！</p>
            </div>
          ) : (
